...
export default {
    ...
    created() {
        ...
        if(isme){
            this.saveBankParam.bankAcctName = bankAcctName?bankAcctName:''
            if(bankAcctName){
    ...
    methods: {             
        ...
        addBankCard(){
            let _this = this;
            if(!/[\u4e00-\u9fa5]{1,}$/.test(_this.saveBankParam.bankAcctName)){
                _this.$dialog({title: '提示信息',message:"请填写用户名",skin: 'ios'});
                return;
            }
            if (!_this.saveBankParam.bankAcct) {
                _this.$dialog({title: '提示信息',message:"请填写银行卡号",skin: 'ios'});
                return;
            };
            ...
            this.saveBankParam.bankProvCity = this.saveBankParam.bankProvCity=='请选择'?'':this.saveBankParam.bankProvCity
