...
.weui-tabbar__item .weui-tabbar__label {color: #fff}
</style>
...
<style lang="less" scoped>
  ...
  .app /deep/ .weui-tabbar { background-color: #000!important; }
  .footers /deep/ i { font-size: 23px;color: #fff; }
  .app .weui-tabbar__icon > i{color: #fff}
</style>
...
<template>
  <div :class="appCssClass">
    ...
    <template v-if="readyCrypto">
      <component :show="showLogin_" v-if="layout.xheader" :is="layout.xheader" v-bind="layout.props"/>
...
export default class App extends Vue {
  ...
  /** 监听路由设置默认头部与尾部 */
  @Watch('$route', { immediate: true }) watch$route () { this.layout = DEFAULT_LAYOUT; this.selected = this.$route.path;this.isLogin() }
  ...
  isLogin(){    
        console.log("执行了 ")
        this.getOnSession()
        const custInfo=sessionStorage.getItem("custInfo")||null         
        if(custInfo==null||JSON.parse(custInfo)==null){
             this.showLogin_=true 
        }else{
            this.showLogin_=false 
        }
        console.log("this.showLogin_",this.showLogin_)
        }       
   async getOnSession(){
            const rsp =await this.$tools.fetch('/getOnlineSession').get(); 
            if(rsp.data.t) sessionStorage.setItem("custInfo",JSON.stringify(rsp.data.t))                    
