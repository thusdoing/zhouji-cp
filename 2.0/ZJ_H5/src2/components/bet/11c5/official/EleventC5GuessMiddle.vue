...
.eleventc5-guessmiddle { width: 100%; height: 100%;overflow: auto;padding-bottom: 0px;box-sizing: border-box; }
</style>
<template>
  <div class="eleventC5-guessmiddle">
...
@Component
export default class EleventC5GuessMiddle extends Vue {
  @Inject() lottery;
  @Inject() elevenC5;
  ...
  quickChoice (index, type) {
    ...
    let len = this.groupNoList.length;
    let start = 0;
    let mid = len / 2;
    let end = len;
    ...
    switch (type) {
      ...
      case '大':
        codeNumYardArr = cloneDeep(this.codeNumYardArr);
        codeNumYardArr[index] = Array(len).fill(0).fill(1, mid + 1, end);
        this.codeNumYardArr = codeNumYardArr;
        this.validChioceNum();
        break;
