...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  LAYIM.prototype.setChatStatus = function(str){
    ...
    if(!thatChat) return;
    var status = thatChat.elem.find('.layim-chat-status2');
  ...
  LAYIM.prototype.setFriendStatus = function(id, type){
    var list = $('.layim-group'+ id);
    list[type === 'online' ? 'removeClass' : 'addClass']('layim-list-gray');
  };
	LAYIM.prototype.sendMsg = function(){
	  sendMessage()
	};
	//设置群禁言
	LAYIM.prototype.setGroupStatus = function(type){
		var data=layim.thisChat().data
		type=type||data.gStatus
		var thatChat=layim.thisChat()
		if(data.type=="group" ){			
			thatChat.textarea[type == '0' ? 'addClass' : 'removeClass']('layim-forbid')
			thatChat.textarea.val(type=="0"?"全员禁言中":"")
			thatChat.textarea.attr("disabled",type=="0"?true:false);
			thatChat.textarea.attr("isForbid",type=="0"?0:1);		
		}
...
// 		console.log(list)
  ...
  var elemTpl = ['<div class="layui-layim-main">'
		,'<div class="layim-user-avater layim-friend{{d.mine.id}}"  data-id="{{d.mine.id}}"> <img src="{{d.mine.avatar}}" class="layui-circle" layim-event="showMine" > <input type="file" name="file" style="opacity:0.01;display:none" id="imgInput"></div>'
    ...
    ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
      ,'<li>'
        ,'<h5 layim-event="spread" lay-type="{{ spread }}" data-id="{{item.id}}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ,'<div class="layui-unselect layim-chat-title">'
      ,'<div class="layim-chat-other">'
        ,'<img class="layim-{{ d.data.type }}{{ d.data.id }} layim-showInfo" src="{{ d.data.avatar }}" layim-event="showInfo"><span class="layim-chat-username" layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||"佚名" }} {{d.data.temporary ? "<cite>临时会话</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>'
        ,'<span class="layim-chat-status layim-chat-status2 layim-chat-username" ></span>'
				,'<p class="layim-chat-status">{{d.data.sign||""}}</p>'
      ,'</div>'
    ,'</div>'
  ...
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
    ,'<div class="layim-chat-user" data-id={{d.userid}} data-avatar={{d.avatar}} data-name={{d.friendNote||d.username||"佚名"}} ><img src="{{ d.avatar }}" {{ d.mine ? "" : "class=layim-showInfo" }} ><cite>'
    ,'{{# if(d.mine){ }}'
      ,'<i>{{ layui.data.date(d.timestamp) }}</i>{{d.friendNote||d.username||"佚名" }}'
     ,'{{# } else { }}'
      ,'{{d.friendNote||d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>'
     ,'{{# } }}'
      ,'</cite></div>'
    ,'<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>'
  ,'</li>'].join('');
  
  var elemChatList = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this layimthis" layim-event="tabChat" data-id={{ d.data.id }} data-type={{ d.data.type }}><img src="{{ d.data.avatar }}"><span>{{d.data.friendNote || d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  var cache = {message: {}, chat: []}, init = function(options){
    ...
    ,obj = {
      ...
      cache = $.extend(cache, obj);		
			localStorage.setItem("mine",JSON.stringify(cache['mine']))
...
/* 				$(".layui-circle").attr("onerror","javascript:this.src='../../layui/images/0.jpg'") */
        //按最新会话重新排列
  ...
  var syncGray = function(data){
    $('.layim-'+data.type+data.id).each(function(){
      if($(this).hasClass('layim-list-gray')){
        layui.layim.setFriendStatus(data.id, 'online'); 
      }
    });
  };
  ...
  var sendMessage = function(){
    ...
    data.content = thatChat.textarea.val();		
		if(thatChat.textarea.attr("isforbid")=="0"){
			return layer.msg("当前群禁止成员发言")
		}
  ...
  var messageNew = {}, getMessage = function(data){
    data = data || {};
		...
		if(data.type === 'group'){
			if(data.userid==1){
					var _gStatus=JSON.parse(data.content).gstatus
					console.log('_gStatus',_gStatus)
					cache.group.forEach((j,item)=>{
						if(j.id==data.id){
							j.gStatus=_gStatus
							if(layim.thisChat()){
								layim.setGroupStatus(j.gStatus)
							}
        ...
        if(data.type === 'friend'){
          ...
          layui.each(cache.friend, function(index1, item1){
            layui.each(item1.list, function(index, item){
              ...
              if(item.id == data.id){
                item.type = 'friend';
                item.name = item.friendNote||item.username;
  ...
  var anim = 'layui-anim-upbit', events = {
        ...
        if(val === ''){
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
              ...
              html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+ data[l].avatar +'"><span>'+ (data[l].friendNote || data[l].username || data[l].groupname || '佚名') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
            }
          } else {
    ...
    ,chat: function(othis){
      ...
      data.name =data.friendNote || data.name || data.username || data.groupname;
      if(type !== 'history'){
        data.type = type;
      }
      popchat(data);
    }
/*    ,chatGroupPerson:function(othis){			
			
			var type="friend",name=othis.data('name'),avatar=othis.data('avatar'),id=othis.data('id')||""
			if(id && id!='undefined'){		
		...
		,showInfo : function(othis){
			var thatChat = thisChat();
					layer.open({
						...
						skin: 'layui-box layui-layer-border',
						resize: true,
						content: layui.cache.dir + 'css/modules/layim/html/myInfo.html'+'?id='+ thatChat.data.id+'&type=friend'
								  
				});
		    /* return layui.data.content(content); */
		  }
    		//查看自己资料
    ,showMine : function(othis){    	
    			layer.open({
    				type:2,
    				title: '个人资料',
    				shade: false,
    				...
    				skin: 'layui-box layui-layer-border',
    				resize: true,
    				content: layui.cache.dir + 'css/modules/layim/html/myInfo.html'+'?id='+ othis.parent().data('id')+'&type=friend'
