	...
	__AjaxFun: function(type, uri, cache, dataType, fun, data) {
		...
		$.ajax({
			...
			error: function(XMLHttpRequest, textStatus, errorThrown) {
				if (XMLHttpRequest.statusText == "timeout") blockUIFun("\u83b7\u53d6\u8d44\u6599\u903e\u65f6\uff0c\u8bf7\u68c0\u67e5\u60a8\u7684\u7ebf\u8def\u72b6\u51b5!!", "Growl");
				else if (typeof fn == "function") fn("error", XMLHttpRequest, textStatus, errorThrown)
			}
		})
...
$(function() {
	...
	if ($.ua.isIe && ($.ua.isIe6 || $.ua.isIe7)) {
		...
		$("#register_form button[type='reset']").click();
		$.validator.addMethod("needOneNumberAndCharter", function(value) {//alert(22);
			return /(?=.*\d)(?=.*[a-z])/i.test(value)
		}, "\u9700\u5305\u542b\u4e00\u4e2a\u82f1\u6587\u5b57\u6bcd\u53ca\u6570\u5b57");
		$.validator.addMethod("onlyNumAndChar", function(value) {
		...
		$("form#register_form").validate({
			rules: {
				...
				signPasswd: {
					required: true,
					minlength: sysConfig.LEASTPASS,
					maxlength: sysConfig.MAXIMUMPASS,
				},
				agpassword: {
					required: true,
					equalTo: "#password",
		...
		$("#register_form").find("input").on("blur keypress", function() {
			$(this).valid()
		});
		$("#register_form").on("click", "#Sreg-button", function() {
			
			if (!$("form#register_form").valid()) return false;
			else {	
				Login.__AjaxFun("post", ddom + "/register", false, "json", "Login.__InitRegister", $("form#register_form").serialize())
			}
		});
		$(".top [title]").tipsy({
			gravity: "n"
