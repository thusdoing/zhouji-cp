...
export default {
    ...
    watch:{
        ...
        schemeCount() {
            this.validChioceNum(this.textAreaV)
        },
        config() {
            this.clearUI()
        }
    ...
    methods:{
        ...
        RemoveRet(){
            var retObj = playMethods.rnzdFilter(this.textAreaV, this.config.sequence, this.config.algorithm)

            if(retObj.repAry.length>0||retObj.errAry.length>0){
                this.$Modal.warning({
                    content:'过滤'+retObj.errAry.length+'个错误号码('+retObj.errAry.join(",")+')，过滤'+retObj.repAry+'个重复号码('+retObj.repAry.join(',')+')',
                })
            }
                ...
                this.$Modal.confirm({
                    ...
                    onOk:function(){
                        if (retObj.retAry.length) {
                            _this.generatebetList(retObj.retAry, arg);
                        }
                        else {
                            _this.social = ['万', '千', '百', '十', '个']
                            _this.textAreaV = ''
                        }
                    },
                    onCancel:function(){
        ...
        validChioceNum(v){ //是否完成有效投注
            v = v.replace(/[^0-9]/ig,"");
            if(v.toString().length>=this.config.algorithm && this.schemeCount > 0){         
                this.betCount = Math.floor(v.length/this.config.algorithm) * this.schemeCount;
        ...
        generatebetList(textAreaV,arg){
            ...
            this.validChioceNum(textAreaV.join(''))
            ...
            let splitArr= textAreaV.map(a => a.split('').join(',')).join('|'); //转换 传入后台输出格式
...
<style scoped lang="less">
    ...
    .local-wrap /deep/ .ivu-checkbox-group {
