<template>
    <div style="width: 1000px;margin: auto;box-sizing: border-box;">
        <!-- <div class="swpbox">
            <swiper :options="swiperOption">
                <swiper-slide style="height: 248px;" v-for="(item, index) in 10" :key="index" @click.native="goingFlag=true">
                    <div class="swpinbox">
                        <img src="/static_pc/img/stopSell.png" alt="">
                        <div class="swpbutton">
                            <div class="lab1">百人牛牛</div>
                            <div class="lab2">创新玩法</div>
                            <div class="lab3">进入游戏</div>
                        </div>
                    </div>
                </swiper-slide>
            </swiper>
        ...
        <div class="qpcont" v-for="(item, index) in plateGameAll" :key="index">
            <div class="qptit">{{item.oPlatName}}</div>
            <div class="coninner">
                <div class="qpitem" @click="goBefore(sub.gamePlateId,sub.id)" v-for="(sub, index2) in item.gList" :key="index2">
                    <div><img style="width: 56px;height: 56px;" :src="$imgPath(sub.gameIoc)" alt=""></div>
                    <div class="innnnner" style="height: 56px; width: 130px;">
...
export default {
    ...
    methods: {
        async getPlateGameAll(){
            ...
            if(res.success){
                this.plateGameAll = res.data.list || []
            }
        },
        goBefore(d,s){
...
.qpcont{border: 1px solid #D8D8D8;margin-top: 20px;}
.qptit{color: rgb(74, 74, 74);font-size: 16px;padding-left: 20px;line-height: 44px;border-bottom: 1px dashed #d8d8d8; }
.coninner{display: flex; padding-top: 30px; flex-wrap: wrap;}
.qpitem{cursor: pointer; display: flex; width: 20%; align-items: center; margin: 0px 0px 30px 40px;}
.innnnner{position: relative;}
.innnnner .lab1,.innnnner .lab2{margin-left: 15px;}
.innnnner .lab1{font-size: 14px;}
...
.qpitem:hover .lab4{display: block}
