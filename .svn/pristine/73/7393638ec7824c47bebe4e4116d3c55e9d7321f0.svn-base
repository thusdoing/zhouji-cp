<template>
    <div class="longhuIntegrate">
        ...
        <div style="overflow:hidden;">
            <div style="width:25%;float:left;" v-for="index in 4" :key="index">
                <Row class="ballGameTitle ballGameDetail">
                    <i-col :span="playModel=='一般' ? 7 : 12">号码</i-col>
                    <i-col :span="playModel=='一般' ? 5 : 12">赔率</i-col>
        ...
        <div v-for="(item1, index) in gameList" :key="'row1'+index" style="overflow:hidden;">
            ...
            <div style="width:25%;float:left;border-right: 1px solid #c9c7c7;border-bottom: 1px solid #c9c7c7;cursor: pointer;" v-for="(item2, pos) in item1.groupSettinges" :key="pos"  :style="{backgroundColor: playModel=='快捷' && item2.selected ? '#FFF8B6': 'white'}" @click="item2.selected = (playModel!='快捷' ? false : !item2.selected);item2.betAmt = item2.betAmt >0 ? '' : kuaijiejine">
                <Row class="aaa" :class="item2.betAmt >0 ?'active':''">
                    <i-col :span="playModel=='一般' ? 7 : 12">
                        <p style="text-align:center;margin-bottom:5px;line-height:36px;">
                            ...
                            <span style="display: block;width: 24px;height: 24px;border: 1px solid #8e8e8e;margin: 8px auto;border-radius: 20px;line-height: 24px;color: #db383f;font-weight: bold;">{{item2.groupName}}</span>
                        </p>
                    </i-col>
                    <i-col :span="playModel=='一般' ? 5 : 12" >
                        <p style="font-size:13px;text-align:center;margin-bottom:5px;color:red;line-height:36px;">{{gameBoundList[index][pos].bonusAmt || '-'}}</p>
                    </i-col>
                    <i-col :span="12" v-if="playModel=='一般'">
                        <div style="padding: 5px 8px;">
                            ...
                            <input class="inputNuber" @click.stop="(()=>{})" @focus="item2.betAmt = item2.betAmt >0 ? '' : kuaijiejine" v-integer type="text" v-model="item2.betAmt"
                        min="0" max="999" />
                        </div>
                    </i-col>
                </Row>
