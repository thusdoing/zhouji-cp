! function(t) {
	...
	103: function(t, n) {
		var e = function(t) {
				function n(t, n) {
					if(1 === arguments.length && "[object Object]" === Object.prototype.toString.call(t)) return $.extend(a, t), {
						gamename: e
					};
					var o = {
						gamename: e,
				...
				switch(GameCode) {
                    ...
                    // case "lhc":
                    //     return i = ["正码1", "正码2", "正码3", "正码4", "正码5", "正码6","特码"], n([1, 47], 7, i);
					case "PK10":
			...
			Vue.component("my-th", {
				...
				methods: {
					switchGame: function(t) {
						...
						//var item = gamelist.find(el => el.id == this.gameID)
						//location.href = location.href.replace(/(game=)\d+/, "$1" + this.gameID).replace(/(type=)[^&]+/, '$1' + item.type)
					},
					tab: function(t) {
					...
					GetData: function(t) {
						...
						clearTimeout(n.rendingTimeout), $.ajax({
							url: n.url + "/gameOpenNo/getLotteryResultList",
							data: $.extend({
								gameID: ('' + n.gameID).substr(0, 4),
								pageSize: n.pageSize,
								pageIndex: 1
							}, t),
							cache: !1,
							success: function(t) {
                                if(GameCode=='lhc'){
                                    n.isLoading = 0
                                    n.lhcData = t.list.length?t.list:[]
                                    n.lhcData = n.lhcData.map(el=>{
                                        var arr = []
                                        arr.push(el.period)
                                        var temparr = el.result.replace('+',',').split(',')
                                        temparr.forEach(sub=>{
                                            getOpenBall(Number(el.gameOpenDt)).forEach(ss=>{
                                                if(ss.num==sub){
                                                    arr.push(ss)
                                                }
                                            })
                                        ...
                                        var sum = temparr.reduce((m,b)=>{
                                            return +m+(+b)
                                        },0)
                                        arr.push(sum)
                                        arr.push(sum>=175?'总大':'总小')
                                        arr.push(sum%2 == 0 ? '总双':'总单')
                                        arr.push(Number(temparr[6])%2==0?'双':'单')
                                        arr.push(Number(temparr[6])<25?'小':'大')
																			arr.push((Number(temparr[6]) % 10 + parseInt(Number(temparr[6])/10))%2==0?'合双':'合单')
																			arr.push((Number(temparr[6]))<26?'合小':'合大')
																			arr.push((Number(temparr[6]) % 10 + parseInt(Number(temparr[6])/10))<5?'合尾小':'合尾大')
                                        if("鸡狗猪牛马羊".indexOf(arr[7].zoo)>=0){
                                            arr.push('家禽')
                                        }else{
                                            arr.push('野兽')
                                        }
								...
								if(n.gameInfo.totalTpl) {
									...
									var e = [],
										...
										u = t.list.length;
									$.ajax({
										url: n.url + "/gameOpenNo/GetCodeRank?gameID=" + n.gameID + "&periodID=" + t.list[0].id,
										cache: !1,
										success: function(c) {
											c = "string" == typeof c ? JSON.parse(c) : c;
											var g = [],
												h = (c.length, n.gameInfo.range[1] - n.gameInfo.range[0] + 1);
											if($.each(c, function(t, e) {
												...
												for(var f = 0; f < n.gameInfo.ballPos * n.gameInfo.ballCount; f++) g[f] = 1;
											...
											$.each(t.list, function(t, n) {
													$.each(n.continuousMissing, function(n, e) {
														e.bgColor = t < u - m[n].count ? "" : '#CCCCEE'; //a[n % d]
													})
												}),
												function() {
													var n = [],
					...
					CanvasDraw: function(t) {
						var n = this,
							e = t.line,
							a = (t.calc, document.createElement("canvas")),
							i = t.width,
							o = t.height,
							s = $("table");
