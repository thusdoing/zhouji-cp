        ...
        <div class="sc-kTUwUJ jSUaeT">
            <div class="sc-bdVaJa kZAgjJ">
                <div class="sc-dqBHgY iijRGP">
                    <div :class="[('sc-cmTdod eCgwLX'),{'ghraAr':selectedBallList.indexOf(index)>=0&&item.select},item.color]" @click="chioceBet(index,item)"
                        v-for="(item,index) in config.ball" :key="index">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            randomChioceBet() {
                this.restBet()
                this.selectedBallList = this.createRandom(this.config.must, 0, this.config.ball.length)
                this.selectedBallList.forEach(el => {
                    this.config.ball[el].select = true
                })
                this.config.ball = this.config.ball.map(el => { return el })
            },
            //投注
            betGame() {
                let _this = this
            ...
            getGameBonusByGroupNo() {
                ...
                this.$http({
                    ...
                    if (res.data.success) {
                        this.lhcBall = res.data.list
                        this.config.ball=this.config.ball.map(el=>{
                            el.groupNo = res.data.list[0].groupNo
                            return el
                        })
                    }
                })
