  ...
  <div class="homepage">
    <swiper class="swiper" height="150px" :show-dots="false" auto>
      <swiper-item class="swiper-item" v-for="d in banners" :key="d.id" v-if="$consts.ImgType.C_BJ == d.imgType">
        <a v-if="d.url"  @click="openUrl(d.url)">
    ...
    <grid class="quickly-navs">
      ...
      <grid-item link="/lixibao" v-if="lxbOpen==1">
        <i slot="icon" class="iconfont icon-qian" style="color: #ffbf35;;"/>
        <span slot="label" style="color: #458dc8;">利息宝</span>
      </grid-item>
...
@Component({
  components: { Swiper, SwiperItem,tanchuang,popUp}
})
export default class HomePage extends Vue {
  ...
  openUrl(url){
    if(this.$ENV.VUE_APP_NAME == '凤凰彩票'){
                        try{
                            return window.webkit.messageHandlers.CPOpenUrl.postMessage(url)
                        }catch(e){
                            (e)=>{}
                        }
                        try{
                            return window.JavaScriptFunction.getUrl(url)
                        }catch(e){
                            (e)=>{}
                        }
  ...
  async getHeadConfig(){
    ...
    let data= rsp.data.list
    data.forEach((item)=>{
      console.log(item)
         localStorage.setItem(item.codeKey,JSON.stringify(item.codeValue)||0)
    })            
    // localStorage.setItem("lxbOpen",JSON.stringify(data[1].codeValue)||0)  
    }else{
      localStorage.setItem("chessOpen",0)
       localStorage.setItem("lxbOpen",0)
    }  
    this.chessOpen=JSON.parse(localStorage.getItem("chessOpen"));
    this.lxbOpen=JSON.parse(localStorage.getItem("lxbOpen"));


  }
  created () {
    this.chessOpen=JSON.parse(localStorage.getItem("chessOpen"))||0;
    this.lxbOpen=JSON.parse(localStorage.getItem("lxbOpen"))||0;
