...
const router = new Router({
  ...
  routes: [
    ...
    { path: '/lixibao', name: 'lixibao', component: require('@/components/custInfos/lixibao/lixibao.vue').default },
    { path: '/transferAcctLXB', name: 'transferAcctLXB', component: require('@/components/custInfos/lixibao/transferAcctLXB.vue').default },
    { path: '/transferRecordLXB', name: 'transferRecordLXB', component: require('@/components/custInfos/lixibao/transferRecordLXB.vue').default },
    { path: '/IncomeReport', name: 'IncomeReport', component: require('@/components/custInfos/lixibao/IncomeReport.vue').default },
    // import explain from '@/components/custInfos/agent/explain.vue'
    ...
    { path: '/custInfo', name: 'custInfo', component: require('@/components/custInfo').default,
      children: [
        ...
        { path: '/recharge/bank', name: 'bankrecharge', component: require('@/components/custInfos/rechargecomp/bankPay.vue').default },
        { path: '/recharge/onl', name: 'thirdPlatformInfo', component: require('@/components/custInfos/rechargecomp/thirdPlatformInfo.vue').default },
        { path: '/recharge/sm', name: 'quickPay', component: require('@/components/custInfos/rechargecomp/quickPay.vue').default },
        // import atmRecharge from '@/components/custInfos/rechargecomp/atmRecharge.vue'//银行卡充值
...
const lxb = ['lxb', 'IncomeReport', 'transferAcctLXB','transferRecordLXB','lixibao']
// 解决页面路由跳转，空白页面，跳转首页，后期提示，首页也有提示，界面弹窗重复
Vue.prototype.isRedirect = false;
const loginInterceptor = async (to, from, next) => {
  const lxbOpen = JSON.parse(localStorage.getItem("lxbOpen")) || 0;
  ...
  console.log(lxb.indexOf(name))
  if (lxb.indexOf(name) != -1){
    if (lxbOpen == 0){
      return next('/');
    }
  }
  ...
  if ( isOk && data.t != null) {
    ...
    if (!isLogged) {
      sessionStorage.setItem("custInfo", JSON.stringify(data.t))    
