...
<template>
    <div>
        ...
        <router-view v-if="$route.params.gameNo"></router-view>
        <div v-else class="container">
            <div class="_10nme">
                <p class="Mjiyg">最新开奖公告</p>
                <table class="_2319D">
                    <thead>
                        <tr class="_3x3A0">
                            <td>彩种</td>
                    ...
                    <tbody>
                        <template v-for='sub in openNOs'>
                            <tr v-if='sub.gameType=="22"||sub.gameType=="33"' class="_2DiFV">
                                <td>{{sub.gameName}}</td>
                                <td>{{sub.gameNumber}}</td>
                                <td v-dateFormat="{a:'yyyy-MM-dd hh:mm:ss',b:sub.gameOpenDt}"></td>
                                <td>
                                    <div class="_24dF_ null">
                                        <img class="mr5" width="20" :src="'/static_pc/images/saizi/sn'+sub.gameOpenNo.split(',')[0]+'.png'"><img class="mr5" width="20" :src="'/static_pc/images/saizi/sn'+sub.gameOpenNo.split(',')[1]+'.png'"><img class="mr5" width="20" :src="'/static_pc/images/saizi/sn'+sub.gameOpenNo.split(',')[2]+'.png'"><span><span><div v-for="xsub in sub.gameOpenNoCase" class="rVgzC">{{xsub}}</div></span></span>
                                    </div>
                                </td>
                                <td>{{sub.gameNote}}</td>
                                <td>{{sub.gameSpan}}</td>
                                ...
                                <td style="cursor: pointer;" @click="moreOp(sub.gameType,sub.gameNo,sub.gameName)">查看更多</td>
                            </tr>
                            <tr v-else class="_2DiFV">
                                ...
                                <td>
                                    <div class="_24dF_ null">
                                        ...
                                        <template v-else>
                                            <div class="wwOAC" :class="{'pAoJe':sub.gameType=='55'}" v-for="(item, index) in sub.gameOpenNo.split(',')" :key="index+'on'">{{item}}</div>
                                        </template>
                                        <span>
                                            <span :style="sub.gameType=='55'?'display:block;':''">
