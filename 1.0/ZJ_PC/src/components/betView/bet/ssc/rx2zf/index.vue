...
export default {
  ...
  methods:{
    ...
    validChioceNum(){ //是否完成有效投注
      ...
      var retAry = algorithm.getTwoArrayIndex(this.codeNumYardArr);
      if (retAry.index.length >= this.config.algorithm) {
        valid = true;
      }
      
      if (valid) {
        this.betCount = playMethods.rnzxfsGetCount(this.codeNumYardArr, this.config.algorithm);
    ...
    btnConfirmChoiceFun(){
      let splitArr = this.codeNumYardArr.map(el => {
        if (el.length) {
          return el.join(',')
        }
        else {
