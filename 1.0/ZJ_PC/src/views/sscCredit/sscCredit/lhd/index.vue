        ...
        <Row>
            <i-col span="24">
                <div class="longhuIntegrateHeader">
                    ...
                    <div style="line-height: 41px;font-weight: bold;">快捷金额设置：
                        <Input v-integer v-model="kuaijiejine" class="kuaijie" size="small" style="width: 60px;"></Input> 元
                        <span>
                            ...
                            <Button type="error" style="width:40px;" @click="kuaijiejine=1000" size="small">1000</Button>
                        </span>
                    </div>
            ...
            <i-col span="6" v-for="(item2, pos) in item1" :key="'col'+index+pos">
                <div style="border:1px solid #ccc;">
                    <p style="padding:5px;text-align:center;background-color:#b62929;color:white;font-weight:bold;">
                        {{ item2.groupClassConName}} 第{{index+1}}球 VS 第{{index + pos + 1 + 1}}球
                    </p>
                    <Row>
                        <i-col span="8" style="padding:0;" v-for="(item3, seat) in item2.groupSettinges" :key="'last'+index+pos+seat">
                            <div style="padding:8px;" class="lh-container" :style="{backgroundColor: playModel=='快捷' && item3.selected ? '#FFF8B6': 'white'}"
                                ...
                                <p style="text-align:center;margin-bottom:5px;">
                                    <label :style="{color:color[seat],fontWeight:'bold',margin:'0 7px'}">{{item3.groupName}}</label>
                                ...
                                <input class="inputNuber" v-show="playModel=='一般'" @click.stop="(()=>{})" @focus="gameList[index][pos].groupSettinges[seat].betAmt = gameList[index][pos].groupSettinges[seat].betAmt >0 ? '' : kuaijiejine"
                                    v-integer type="text" v-model="gameList[index][pos].groupSettinges[seat].betAmt" min="0"
                                    max="999" />
                ...
                <Row>
                    <i-col :span="24">
                        <p style="text-align:center;">共
                            ...
                            <span style="color:red;font-weight:bold;">{{totalAmt}}
                            </span>元</p>
                    </i-col>
                </Row>
            </div>
        ...
        computed: {
            ...
            totalAmt:function(){
                if(this.playModel=='快捷'){
                    return this.allAmt*this.betRetList.length
                }else{
        ...
        methods: {
            init() {
                var temData = _.cloneDeep(this.gameData),
                    ...
                    temBound = [];
                temList.push(
                    temData.groupClassCones.filter(el => {
                        return el.groupClassConName[0] == "万";
                    })
                );

                temList.push(
                    temData.groupClassCones.filter(el => {
                        return el.groupClassConName[0] == "千";
                    })
                );

                temList.push(
                    temData.groupClassCones.filter(el => {
                        return el.groupClassConName[0] == "百";
                    })
                );

                temList.push(
                    temData.groupClassCones.filter(el => {
                        return el.groupClassConName[0] == "十";
                    })
                );
                    ...
                    elAry1.forEach((elAry2, pos) => {
                        ...
                        elAry2.groupSettinges.forEach((el, seat) => {
                            if (this.playModel == "快捷") {
                                if (el.selected) {
                                    betList.push({
                                        content: `第${index + 1}球 VS 第${index + pos + 1 + 1}球: ${
                                            this.lhh[seat]
                                            }`,
                                        groupNo: el.groupNo,
            ...
            async onBet() {
                ...
                var postData = this.betRetList.map(el => {
                    return {
                        ...
                        betBonus: this.gameBoundList[el.betBonusIndex[0]][
                            el.betBonusIndex[1]
                        ][el.betBonusIndex[2]].bonusAmt
                    };
                });
